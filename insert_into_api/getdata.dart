...
class _DataGetState extends State<DataGet> {
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        courses = List<Data>.from(
          courseData.map((course) => Data.fromJson(course)).toList().reversed,
        );
      });
    } else {
        ...
        itemBuilder: (context, index) {
          return Card(
              child:Column(
                ...
                children: [
                  Text('User_id : ${courses[index].userId}'),
                  Text(' ${courses[index].courseName!}'),
                  Text('Credit hours : ${courses[index].creditHours!}'),
                  Text('Semester : ${courses[index].semesterNo!}'),
                  Text('Marks: ${courses[index].marks!}'),
                ],
              )
          );
        },
